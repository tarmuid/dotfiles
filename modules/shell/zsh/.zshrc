...
dot_active_modules_file="${XDG_STATE_HOME}/dot/active-modules"
if [[ -f "$dot_active_modules_file" ]]; then
  while IFS= read -r dot_module; do
    [[ -z "$dot_module" || "$dot_module" = \#* ]] && continue

    dot_alias_file="$DOTFILES_DIR/modules/${dot_module}/aliases.zsh"
    [[ -f "$dot_alias_file" ]] && source "$dot_alias_file"
  done < "$dot_active_modules_file"
fi
